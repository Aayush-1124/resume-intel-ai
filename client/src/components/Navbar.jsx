...
import { Sparkles, Sun, Moon, LayoutDashboard, LayoutTemplate, FileCheck2, Briefcase } from 'lucide-react';
...
export default function Navbar({ activePage, activeStep, onNavigate, onOpenTemplates, isDark, onThemeToggle }) {
  return (
    <nav
      className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-xl border-b border-outline-variant/10 flex justify-between items-center w-full px-8 py-3.5"
    ...
    >
      ...
      <div className="flex items-center gap-10">
        <button
          ...
          className="flex items-center gap-2.5 text-[1.35rem] font-extrabold text-on-surface tracking-tighter hover:opacity-80 transition-opacity"
        ...
        {/* Nav links */}
        <div className="hidden md:flex gap-1.5 bg-surface-container-low p-1.5 rounded-[1.25rem] border border-outline-variant/20 shadow-inner" role="navigation" aria-label="Site sections">
          {NAV_LINKS.map((link) => {
      ...
      <div className="flex items-center gap-4">
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          className="px-6 py-2.5 bg-gradient-to-r from-primary to-primary-container text-on-primary font-bold text-sm tracking-wide rounded-full shadow-lg hover:shadow-xl transition-all"
